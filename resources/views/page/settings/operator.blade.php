                  ...
                  <tr class="gradeU">
                    ...
                    <td>
                      <a data-toggle="modal" data-target="#update" data-id="{{$data->id}}"
                        data-provider="{{$data->n_operator}}" data-status="{{$data->status}}"
                      ...
                      <a href="#" type="button" class="btn btn-sm mb-2 btn-danger delete" provider-id="{{$data->id}}"
                        provider-nama="{{$data->n_operator}}"><i class="fa fa-trash-o"></i></a>
                    </td>
                  </tr>
                  @endforeach
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form role="form" method="POST" action="{{ url('operator/simpan') }}" data-toggle="validator"
          enctype="multipart/form-data" role="form">
          @csrf
          <div class="form-group">
            <label for="exampleInputEmail1">Nama Operator</label>
            <input type="text" name="n_operator" class="form-control" id="exampleInputEmail3"
              placeholder="Nama Operator Seluler">
          </div>
          <div class="form-group">
...
<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="update" class="modal fade">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Update Operator</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        ...
        <form role="form" method="POST" action="{{ url('operator/update/id') }}" data-toggle="validator"
          enctype="multipart/form-data" role="form">
          @method('patch')
          @csrf
          <div class="form-group">
            ...
            <input type="text" name="n_operator" class="form-control" id="operator" placeholder="Nama Operator">
          </div>
...
<script>
  $('.delete').click(function(){
    ...
    var operator_nama = $(this).attr('operator-nama');
    swal({
      title: "Yakin ?",
      text: "Mau di Hapus data dengan nama "+operator_nama+" ??",
      icon: "warning",
      buttons: true,
      dangerMode: true,
      })
      .then((willDelete) => {
      if (willDelete) {
        window.location = "/operator/delete/" + provider_id;
